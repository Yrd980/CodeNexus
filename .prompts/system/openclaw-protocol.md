...
```text
1. OpenClaw 发现候选项目
2. 用 repo-scorer.md 判断是否值得深入研究
3. 用 code-extractor.md 分析项目 → 得到分析报告 JSON
4. 用 verification-rubric.md 检查验证证据是否足够
5. 用 pr-opportunity-scorer.md 判断是否存在高价值 PR 机会
6. 用 code-refiner.md 炼化代码 → 得到标准模块 / 知识卡片 / 伪代码模式
7. 如需回馈原项目，先生成 draft proposal，人工审核后再决定是否提 PR
8. 向 CodeNexus 提 PR，分支命名：openclaw/<module-path>
9. CodeNexus 用 pr-reviewer.md 审核
10. 记录结果与反馈，更新下一轮判断标准
```
